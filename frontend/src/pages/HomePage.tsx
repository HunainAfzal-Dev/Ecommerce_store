...
export default function HomePage() {
  ...
  return (
    <div className="space-y-12 sm:space-y-16">
      ...
      <section className="relative bg-[var(--color-surface-subtle)] border-b border-stone-200/90 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20 lg:py-28">
          <div className="max-w-2xl space-y-6">
            <div className="inline-flex items-center space-x-2 bg-blue-50 text-blue-700 border border-blue-200 text-xs uppercase tracking-widest font-bold px-3 py-1.5 rounded-md shadow-2xs">
            ...
            <p className="text-sm sm:text-base text-stone-600 font-normal leading-relaxed max-w-lg">
              Explore our curated garments crafted from pure breathable fabrics, tailored for effortless daily wear and understated modern luxury.
            </p>
            ...
            <div className="pt-2 flex flex-wrap items-center gap-3 sm:gap-4">
              ...
              <Link
                to="/shop"
                className="bg-white hover:bg-stone-50 text-stone-900 border border-stone-300 text-xs uppercase tracking-wider font-bold px-7 py-3.5 rounded-lg shadow-2xs transition-all duration-200"
              >
                View Catalog &rarr;
              </Link>
            </div>
      ...
      {/* 2. PRODUCT CARDS MUST BE DIRECTLY BELOW HERO SECTION */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-2">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 pb-3 border-b border-stone-200">
          ...
          <Link
            to="/shop"
            className="text-xs uppercase tracking-wider font-bold text-stone-700 hover:text-stone-950 transition mt-2 sm:mt-0 flex items-center gap-1 group"
          >
      ...
      {/* 3. Shop by Category Showcase with Dashboard-Style Semantic Accent Cards */}
      {categories.length > 0 && (
            ...
            {categories.slice(0, 8).map((category, idx) => {
              ...
              return (
                <Link
                  ...
                  className={`group bg-white border border-stone-200/90 rounded-xl p-5 sm:p-6 flex flex-col justify-between min-h-[150px] hover:border-stone-400 shadow-xs hover:shadow-sm transition-all duration-300 relative overflow-hidden`}
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className={`inline-flex items-center gap-1 text-[10px] uppercase tracking-wider font-bold ${theme.text} ${theme.bg} border ${theme.border} px-2 py-0.5 rounded-md`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${theme.dot}`}></span>
                        Department
                      </span>
                    </div>

                    <h3 className={`text-base sm:text-lg font-bold text-stone-950 ${theme.hoverText} transition-colors`}>
                      {category.name}
                    </h3>
                  ...
                  <div className={`pt-3 flex items-center justify-between text-xs font-bold uppercase tracking-wider ${theme.text}`}>
                    <span>Explore</span>
                    <span className="group-hover:translate-x-1 transition-transform">&rarr;</span>
                  </div>
                </Link>
      ...
      {/* 4. Brand Value Pillars with Subtle Semantic Accents */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 bg-white border border-stone-200/90 rounded-2xl p-6 sm:p-8 shadow-xs">
